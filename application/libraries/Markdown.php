...
class Markdown {
	...
	//获取总页数
	public function getTotalPages($pageSize, $lang) {
		$allblogs = $this->blogs;
		foreach ($allblogs as $key => $bloglist) {
			if($bloglist['language'] !== $lang) {
				unset($allblogs[$key]);
			}
		}
	...
	//新闻分页列表
	public function getBlogsByPage($pageNo, $pageSize, $lang) {
		$allblogs = $this->blogs;
		foreach ($allblogs as $key => $bloglist) {
			if($bloglist['language'] !== $lang) {
				unset($allblogs[$key]);
		...
		$pages = ceil($total / $pageSize);
		$offset = ($pageNo - 1) * $pageSize;
		$blogList = array_slice($allblogs, $offset, $pageSize);
	...
	private function readPostBaseInfo($serverPath) {
		...
		$noteTmpArr = array();
		$pattern1 = '/<\!\-\-(.*?)\-\->/is';
		$pattern2 = '/^\s*(author|head|date|title|summary|images|tags|category|status|language)\s*:(.*?)$/im';
	...
	private function readAllPostInfo($mdfiles, $postPath) {
		foreach ($mdfiles as $idx => $fileProp) {
			...
			$siteURL = "/news/" . $this->changeFileExt($relativePath);
